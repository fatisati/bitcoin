...
long block_header_l;
int p =1;
unsigned long H0[100]; //be andazeye tedad 512 taii ha
...
void int_padding(int msg[], int length, int res[]){ //msb = 0
...
long binary_to_long(int binary[], int length){
    ...
    for(i=0; i<length; i++){
		...
		//long p1 = power(2, length - i - 1);
		//long p2 = pow(2, length - i - 1);

        res += power(2, length - i - 1)*binary[i];
    }
    return res;
}
...
void sha256(int msg[], int length, int w[64][32], int hash_values[2][256]){
...
int main()
{
    ...
    printf("before hash\n");
    sha256(msg, length, w, hash_values);
    printf("hash completed\n");

    for(i=0; i<256; i++){
        printf("%d", hash_values[p-1][i]);
    }
